  ...
  useEffect(() => {
    ...
    const loadAnalytics = async () => {
      try {
        ...
        const todayTripsCount = todayReports?.reduce(
          (sum, r) => sum + (r.expense_count || 0),
          0
        ) || 0;

        const todayRevenue = todayReports?.reduce(
          (sum, r) => sum + (r.net_after_expenses || 0),
          0
        ) || 0;

        const todayExpenses = todayReports?.reduce(
          (sum, r) => sum + (r.expense_count || 0),
          0
        ) || 0;
        ...
        const yearRevenue = yearReports?.reduce(
          (sum, r) => sum + (r.net_after_expenses || 0),
          0
        ) || 0;

        const yearGrossCost = yearReports?.reduce(
          (sum, r) => sum + (r.expense_count || 0),
          0
        ) || 0;
